...
Each element has an independent complex weight $w_n \in \mathbb{C}$. No structure is imposed on the weights; this is the most general excitation model.

The array factor is complex and the modulus constraint $|AF(\hat{r}_p)| \leq t_p$ is approximated by a polygon (LP, QP) or imposed exactly as a second-order cone (SOCP).
