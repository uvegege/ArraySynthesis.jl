...
For a centrosymmetric array (positions in symmetric pairs $\pm\mathbf{p}_n$), enforcing $w_{-n} = w_n^*$ makes the array factor **always real**:
...
Only the $\lceil N/2 \rceil$ representative weights (stored in `SymmetricArray`) are optimization variables.

**Advantages over `ComplexWeights`:**
- The modulus constraint $|AF| \leq t$ reduces to $-t \leq AF \leq t$: exact and linear in all formulations, with no polygon approximation error.
