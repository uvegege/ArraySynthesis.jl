...
Uses the same polyhedral (linear) modulus constraint as `LP`. The difference is the objective: `QP` allows a quadratic term, enabling objectives such as `MinPower`, `MinIntegratedPower`, and `MinFieldError`. The constraints themselves are always linear.
